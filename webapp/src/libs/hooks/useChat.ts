...
export const useChat = () => {
    ...
    const createChat = async () => {
        const chatTitle = `${Constants.bot.profile.fullName} @ ${new Date().toLocaleString()}`;
        try {
            await chatService
...
export function getFriendlyChatName(convo: ChatState): string {
    ...
    const autoGeneratedTitleRegex = new RegExp(`${Constants.bot.profile.fullName} @ [0-9]{1,2}\/[0-9]{1,2}\/[0-9]{1,4}, [0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2} [A,P]M`);
